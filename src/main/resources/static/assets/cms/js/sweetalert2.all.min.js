...
document.getElementById('form-search-global').addEventListener('input', function () {
    let tukhoa = removeDiacritics(this.value.toLowerCase());
    document.querySelectorAll('.funtion-name').forEach(element => {
        let name = removeDiacritics(element.textContent.trim().toLowerCase());
        let dropdownItem = element.closest('.dropdown-item');
        if (name.includes(tukhoa)) {
...
document.querySelectorAll('.function-item').forEach(function (element) {
    element.addEventListener('click', function () {
        let link = this.getAttribute('href');
        let name = this.querySelector('span.funtion-name').textContent;
        let arrSearch = localStorage.getItem('recent-searches');
        arrSearch = arrSearch ? JSON.parse(arrSearch) : [];
        let shouldContinue = true;
        if (arrSearch.length > 0 && arrSearch[0].name === name) {
            shouldContinue = false;
        }
        if (shouldContinue) {
            if (arrSearch.length >= 2) {
                arrSearch.splice(1, 1);
            }
            arrSearch.unshift({link: link, name: name});
            localStorage.setItem('recent-searches', JSON.stringify(arrSearch));
        }
    });
});
...
function printSearch() {
    let arrSearch = localStorage.getItem('recent-searches');
    arrSearch = arrSearch ? JSON.parse(arrSearch) : [];
    let reccent = document.getElementById('recent-searches');
    reccent.innerHTML = '';
    let span = '';
    arrSearch.forEach(item => {
        span += `<span class="h4 mr-1">
                             <a class="btn btn-xs btn-soft-dark btn-pill" href="${item.link}">
                                 ${item.name} <i class="tio-search ml-1"></i>
                             </a>
                        </span>`;
    });
    reccent.innerHTML = span;
}